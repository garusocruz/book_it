...
@router.get("/place/{place_id}", response_model=list[schemas.Calendar])
def read_calendar(current_user: Annotated[user_schema.User, Depends(user_service.get_current_active_user)], place_id: int, db: Session = Depends(get_db)):
    db_calendar = crud.get_calendar_by_place_id(db, place_id=place_id)
    if db_calendar is None:
        raise HTTPException(status_code=404, detail="Calendar not found")
    return db_calendar
